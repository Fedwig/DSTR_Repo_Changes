...
public:
    ...
    car* searchCarIDBinary() {
        ...
        car* car = binaryCarSearch(carIDInput);
    ...
    void modifiedInsertionSort(string sortFactor) {
        if (carHead == nullptr || carHead->carNext == nullptr) {
            return;
        }
        car* sorted = nullptr;
        car* current = carHead;
        while (current != nullptr) {
            car* next = current->carNext;
            if (sorted == nullptr || compareCars(current, sorted, sortFactor)) {
                current->carNext = sorted;
                sorted = current;
            }
            else {
                car* temp = sorted;
                while (temp->carNext != nullptr && !compareCars(current, temp->carNext, sortFactor)) {
                    temp = temp->carNext;
                }
                current->carNext = temp->carNext;
                temp->carNext = current;
            }
            current = next;
        }
        carHead = sorted;
    ...
    bool compareCars(car* car1, car* car2, string sortFactor) {
        if (sortFactor == "carID") { //1
            return car1->carID < car2->carID;
        }
        else if (sortFactor == "carPrice") {//2
            return car1->carPrice < car2->carPrice;
        }
        else if (sortFactor == "carRegistrationDate") {//3
            return car1->carRegistrationDate < car2->carRegistrationDate;
        }
        else if (sortFactor == "carMileage") {//4
            return car1->carMileage < car2->carMileage;
        }
        else if (sortFactor == "fuelType") {//5
            return car1->carFuelType < car2->carFuelType;
        }
        else if (sortFactor == "transmission") {//6
            return car1->carTransmission < car2->carTransmission;
        }
        else if (sortFactor == "carEngineSize") {//7
            return car1->carEngineSize < car2->carEngineSize;
        }
        else if (sortFactor == "carDoors") {//8
            return car1->carDoorNum < car2->carDoorNum;

        }
        else if (sortFactor == "carColour") {//9
            return car1->carColour < car2->carColour;
        }
        else if (sortFactor == "carURL") {//10
            return car1->carURL < car2->carURL;
        }
        else if (sortFactor == "carBodyType") { // 11
            return car1->carBodyType < car2->carBodyType;
        }
        else if (sortFactor == "saleDate") { // 12
            return car1->carInitialSaleDate < car2->carInitialSaleDate;
        }
        else if (sortFactor == "carStatus") { // 13
            return car1->carStatus < car2->carStatus;
        }
        else {
            // default case: sort by carID
            return car1->carID < car2->carID;
        }
    }
    ...
    void searchAvailableCars() {
        ...
        modifiedInsertionSort("carRegistrationDate");

        cout << "\tSearching for '" << status << "': " << endl;

        while (current != NULL) {
            if (current->carStatus == status) {
                count++;
        ...
        if (count == 0) {
            cout << "\tNo available cars found." << endl;
        }
        modifiedInsertionSort("carID");
        cout << endl;
    }



    void searchTitleBasedOnKeyword()
    {
        car* current = carHead;
        string keyword;
        int i = 0;
...
public:
    ...
    customer* insertToTheEndCustomerList(string customerID, string customerName, string customerPhoneNum, double customerDeposit)
    {

        customer* newCustNode = createNewCustomerNode(customerID, customerName, customerPhoneNum, customerDeposit);
    ...
    void storeinCustomerLinkedList(string fileName) {
        ...
        while (getline(file, line)) {
            ...
            insertToTheEndCustomerList(newCustNode->customerID, newCustNode->customerName, newCustNode->customerPhoneNum, newCustNode->customerDeposit);
        }
        return;
    }
    ...
    //// search on linked list
    customer* binaryCustomerSearch(string custid)
    {
        struct customer* head = custHead;
        struct customer* tail = custTail; 
    ...
    void bubbleSortCustomerID() {
        ...
        do {
            ...
            while (ptr1->custNext != lptr) {
                if (ptr1->customerID > ptr1->custNext->customerID) {
                    swap(ptr1, ptr1->custNext);
                    swapped = true;
                }
                ptr1 = ptr1->custNext;
            }
    ...
    void bubbleSortCustomerName() {
        ...
        do {
            ...
            customer* current = custHead;
            while (current->custNext != NULL) {
                if (current->customerName > current->custNext->customerName) {
                    // Swap the nodes
                    temp = current->custNext;
                    current->custNext = current->custNext->custNext;
                    temp->custNext = current;
                    if (current == custHead) {
                        custHead = temp;
                    }
                    swapped = true;
    ...
    void bubbleSortInvoiceID() {
        salesInvoice* temp = NULL;
        bool swapped;

        // Check if list is empty
        if (saleInvoiceHead == NULL) {
            return;
        }
        ...
        do {
            ...
            while (current->invoiceNext != NULL) {
                if (current->invoiceID > current->invoiceNext->invoiceID) {
                    // Swap the nodes
                    temp = current->invoiceNext;
                    current->invoiceNext = current->invoiceNext->invoiceNext;
                    temp->invoiceNext = current;
                    if (current == saleInvoiceHead) {
                        saleInvoiceHead = temp;
                    }
                    swapped = true;
                }
                else {
    ...
    void bubbleSortInvoiceCustomerID() {
        salesInvoice* temp = NULL;
        bool swapped;
        ...
        do {
            ...
            salesInvoice* current = saleInvoiceHead;
            while (current->invoiceNext != NULL) {
                if (current->customerID > current->invoiceNext->customerID) {
                    // Swap the nodes
                    temp = current->invoiceNext;
                    current->invoiceNext = current->invoiceNext->invoiceNext;
                    temp->invoiceNext = current;
...
public:
    ...
    int displayCarListItemsBinary() {
        ...
        {
            cout << "\n\tCar has already been booked." << endl;
    ...
    manageCustomerSection:
        ...
        cout << "\t||                                      ||" << endl;
        cout << "\t||   3. SEARCH CUSTOMER DETAILS         ||" << endl;
        cout << "\t||                                      ||" << endl;
    ...
    void addNewCustomer() {
        ...
        //newNode = createNewCustomerNode(custID, custName, custPhoneNum, NULL);
        insertToTheEndCustomerList(custID, custName, custPhoneNum, NULL);
    ...
    void updateCustomerCSV()
    {
        // Open the CSV file in append mode
        customer* current = custHead;
        ofstream csvFile;
        csvFile.open("customer.csv");
        csvFile << "Customer ID, Customer Name, Customer Phone Number, Customer Deposit\n";
        while (current != NULL) {
            csvFile << current->customerID << ","
                << current->customerName << ","
                << current->customerPhoneNum << ","
                << current->customerDeposit << std::endl;
    ...
    void searchCustomerDetails() {
        ...
        customer* customer = binaryCustomerSearch(custID);
